...
MATCH='\[Interface\]'
ADD1='PostUp = DROUTE=\\\$(ip route | grep default | awk \\\"{print \\\$3}\\\"); HOMENET=192.168.0.0/16; HOMENET2=172.16.0.0/12; ip route add \\\$HOMENET2 via \\\$DROUTE; ip route add \\\$HOMENET via \\\$DROUTE; iptables -I OUTPUT -d \\\$HOMENET -j ACCEPT; iptables -A OUTPUT -d \\\$HOMENET2 -j ACCEPT; iptables -A OUTPUT ! -o pia -m mark ! --mark \\\$(wg show pia fwmark) -m addrtype ! --dst-type LOCAL -j REJECT'
